...
<body>
	<div id="wrap">
							...
							<tbody>
								<c:forEach items="${requestScope.boardList}" var="board">
									<tr>
										...
										<td>${board.regDate}</td>
										<c:choose>
											<c:when test="${not empty authUser and authUser.no == board.userNo}">
												<td><a href="${pageContext.request.contextPath}/board/delete?no=${board.no}">[삭제]</a></td>
											</c:when>
											<c:otherwise>
												<td></td>
											</c:otherwise>
		...
		<div id="footer">Copyright ⓒ 2020 이예슬. All right reserved</div>
		<!-- //footer -->
	</div>
	<!-- //wrap -->
